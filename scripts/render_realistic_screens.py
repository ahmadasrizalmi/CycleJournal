...
def render_screens():
    ...
    # TopAppBar
    logo_small = logo_master.resize((54, 54))
    im1.paste(logo_small, (margin, 82), logo_small)
    d1.text((margin + 68, 86), "CycleJournal", fill=c_slate_900, font=font_bold_title)

    # Hero Card (Gradient)
    draw_gradient_rect(d1, margin, 165, width - margin, 475)
    d1.text((margin + 35, 195), "Hari ke-14 Siklus", fill=c_white, font=font_bold_title)
    # Pill
    d1.rectangle([margin + 35, 265, margin + 225, 310], fill=(255, 255, 255, 60))
    d1.text((margin + 55, 275), "JENDELA SUBUR", fill=c_white, font=font_caption)
    d1.text((margin + 35, 340), "Perkiraan Ovulasi dalam 2 hari (Akurasi FIGO)", fill=(255, 241, 242, 255), font=font_body)
    d1.text((margin + 35, 390), "Panjang Siklus Rata-rata: 28 Hari (±1.5 Hari)", fill=(255, 241, 242, 220), font=font_caption)

    # Month Navigator
    d1.text((margin, 520), "<", fill=c_slate_600, font=font_section)
    d1.text((width // 2 - 110, 520), "September 2026", fill=c_slate_900, font=font_section)
    d1.text((width - margin - 20, 520), ">", fill=c_slate_600, font=font_section)
    ...
    d1.rectangle([margin, 1340, width - margin, 1660], fill=(248, 250, 252, 255), outline=c_border, width=2)
    d1.text((margin + 35, 1375), "Catatan Hari Ini (14 Sep 2026)", fill=c_slate_900, font=font_section)
    d1.text((margin + 35, 1435), "• Aliran Haid: Tidak ada perdarahan", fill=c_slate_600, font=font_body)
    d1.text((margin + 35, 1485), "• Suhu Basal (BBT): 36.45 °C (Baseline Shift)", fill=c_slate_600, font=font_body)
    d1.text((margin + 35, 1535), "• Lendir Serviks: Putih Telur (Peak Fertility)", fill=(14, 116, 144, 255), font=font_body)
    d1.text((margin + 35, 1585), "• Skala Nyeri: 0 / 10 (Bebas nyeri)", fill=c_slate_600, font=font_body)

    # Bottom Sticky Action Button
    draw_gradient_rect(d1, margin, 1720, width - margin, 1830)
    d1.text((width // 2 - 140, 1755), "Catat Gejala Hari Ini", fill=c_white, font=font_section)
    save_image_safe(im1, os.path.join(raw_dir, "dashboard.png"))
    print("Saved refined dashboard.png")
    ...
    for i, (label, active) in enumerate(flow_chips):
        ...
        if active:
            ...
            d2.text((cx1 + (chip_w - 60) // 2, 470), label, fill=c_white, font=font_sub_bold)
        else:
            d2.rectangle([cx1, 445, cx2, 520], fill=(248, 250, 252, 255), outline=c_border, width=2)
            d2.text((cx1 + (chip_w - 60) // 2, 470), label, fill=c_slate_900, font=font_sub_bold)
    ...
    # Slider Track
    d2.rectangle([margin, 630, width - margin, 646], fill=(241, 245, 249, 255))
    active_slider_w = int((width - margin * 2) * 0.7)
    d2.rectangle([margin, 630, margin + active_slider_w, 646], fill=(220, 38, 38, 255))
    d2.ellipse([margin + active_slider_w - 16, 638 - 16, margin + active_slider_w + 16, 638 + 16], fill=(220, 38, 38, 255))
    ...
    d2.rectangle([margin, 725, width - margin, 845], fill=(254, 242, 242, 255), outline=(239, 68, 68, 255), width=2)
    ...
    d2.rectangle([margin, 880, width // 2 - 15, 990], fill=c_white, outline=c_border, width=2)
    ...
    d2.rectangle([width // 2 + 15, 880, width - margin, 990], fill=(248, 250, 252, 255), outline=c_border, width=2)
    ...
    for i, (label, active) in enumerate(mucus_chips):
        ...
        if active:
            ...
            d2.text((cx1 + (m_chip_w - 90) // 2, 1110), label, fill=(14, 116, 144, 255), font=font_sub_bold)
        else:
            d2.rectangle([cx1, 1085, cx2, 1160], fill=(248, 250, 252, 255), outline=c_border, width=2)
            d2.text((cx1 + (m_chip_w - 70) // 2, 1110), label, fill=c_slate_900, font=font_sub_bold)

    # Section 5: Notes
    d2.text((margin, 1200), "Catatan Tambahan", fill=c_slate_900, font=font_section)
    d2.rectangle([margin, 1250, width - margin, 1420], fill=c_white, outline=c_border, width=2)
    d2.text((margin + 25, 1280), "Nyeri pelvis terasa sejak pagi, konsumsi paracetamol 500mg...", fill=c_slate_900, font=font_body)

    # Save Button
    draw_gradient_rect(d2, margin, 1720, width - margin, 1830)
    d2.text((width // 2 - 140, 1755), "Simpan Catatan Harian", fill=c_white, font=font_section)

    save_image_safe(im2, os.path.join(raw_dir, "daily-log.png"))
    print("Saved refined daily-log.png")
    ...
    # A4 Paper Mock Card
    d3.rectangle([margin, 160, width - margin, 1480], fill=c_white, outline=(226, 232, 240, 255), width=2)
    ...
    d3.rectangle([margin + 30, 410, width - margin - 30, 520], fill=(248, 250, 252, 255), outline=c_border, width=1)
    ...
    d3.rectangle([margin + 30, 610, width - margin - 30, 710], fill=(254, 242, 242, 255), outline=(239, 68, 68, 255), width=2)
    ...
    d3.rectangle([margin + 30, 800, width - margin - 30, 850], fill=(241, 245, 249, 255))
    ...
    d3.rectangle([margin + 30, 1220, width - margin - 30, 1420], outline=c_border, width=2)
    ...
    # Export CTA Buttons
    draw_gradient_rect(d3, margin, 1540, width - margin, 1650)
    d3.text((width // 2 - 140, 1575), "Ekspor PDF Siap SpOG", fill=c_white, font=font_section)

    d3.rectangle([margin, 1680, width - margin, 1790], outline=c_coral, width=3)
    d3.text((width // 2 - 160, 1715), "Ekspor Data CSV (Excel)", fill=c_coral, font=font_section)

    save_image_safe(im3, os.path.join(raw_dir, "medical-report.png"))
    print("Saved refined medical-report.png")
    ...
    # Card 1: Anonymous ID
    d4.rectangle([margin, 160, width - margin, 310], fill=(255, 248, 248, 255), outline=(255, 228, 230, 255), width=2)
    ...
    d4.rectangle([margin, 400, width - margin, 680], fill=c_white, outline=c_border, width=2)
    ...
    draw_gradient_rect(d4, margin + 30, 580, width // 2 - 15, 650)
    d4.text((margin + 70, 600), "Cadangkan", fill=c_white, font=font_sub_bold)

    d4.rectangle([width // 2 + 15, 580, width - margin - 30, 650], outline=c_border, width=2)
    ...
    d4.rectangle([margin, 770, width - margin, 1050], fill=c_white, outline=c_border, width=2)

    d4.text((margin + 30, 810), "Kunci Sidik Jari (Biometrik)", fill=c_slate_900, font=font_sub_bold)
    d4.rectangle([width - margin - 100, 805, width - margin - 30, 845], fill=c_pink)
    ...
    d4.rectangle([margin, 1140, width - margin, 1450], fill=(254, 242, 242, 255), outline=(239, 68, 68, 255), width=2)
    ...
    d4.rectangle([margin + 30, 1330, width - margin - 30, 1420], fill=(239, 68, 68, 255))
    d4.text((width // 2 - 200, 1355), "HAPUS SEMUA DATA PERMANEN", fill=c_white, font=font_sub_bold)

    save_image_safe(im4, os.path.join(raw_dir, "privacy-security.png"))
    print("Saved refined privacy-security.png")
    ...
    # Badge pill
    d5.rectangle([width // 2 - 250, 560, width // 2 + 250, 610], fill=(255, 241, 242, 255))
    d5.text((width // 2 - 220, 575), "100% OFFLINE • ZERO-KNOWLEDGE • STANDAR FIGO", fill=(190, 18, 60, 255), font=font_caption)

    # Disclaimer Card
    d5.rectangle([margin, 660, width - margin, 980], fill=(248, 250, 252, 255), outline=c_border, width=2)
    ...
    # Final Button
    draw_gradient_rect(d5, margin, 1720, width - margin, 1830)
    ...
    print("Saved refined data-portability.png (onboarding)")
